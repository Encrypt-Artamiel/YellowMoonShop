...
public class UpdateCartServlet extends HttpServlet {
    ...
    private final String HOME_PAGE = "user.jsp";
    private final Logger LOG = Logger.getLogger(UpdateCartServlet.class);
        ...
        try {
            ...
            if (session != null) {
                ...
                if (cart != null) {
                    ...
                    boolean check = productDAO.checkValidProduct(productID, newQuantity);
                    if (check) {
                        cart.updateQuantityCake(productID, newQuantity);
                        cart.caculateTotal();
                    } else {
                        productDAO.getProductByID(productID);
                        ProductDTO cake = productDAO.getProduct();
                        request.setAttribute("QUANTITY_IN_STOCK", cake.getQuantity());
                    }
                }
                session.setAttribute("CART", cart);
            }
        } catch (NumberFormatException ex) {
            String err = "Please enter right format!!";
